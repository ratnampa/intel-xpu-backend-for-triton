#ifndef TRITON_DIALECT_TRITON_INTEL_GPU_IR_DIALECT_H_
#define TRITON_DIALECT_TRITON_INTEL_GPU_IR_DIALECT_H_
...
//// elements. If you want non-replicated warps, use
/// getWarpsPerCTAWithUniqueData.
...
//// 1]. Whereas for a tensor shape [128, 128], the elements for thread 0 would
/// be / [A_{0, 0}, A_{0, 1}, A_{0, 2}, A_{0, 3}], returns [1, 4].
...
//// 1], threadsPerWarp = [2, 16] and tensor shape = [2, 2], threads 0, 1, 16,
/// 17 / have access to the full tensor, whereas the other threads have access
/// to / replicated elements, so this function returns [2, 2].
...
//// elements of the tensor. E.g. for a blocked layout with sizePerThread = [1,
//// 1], threadsPerWarp = [2, 16], warpsPerCTA = [1, 4] and tensor shape = [2,
/// 2], / returns [1, 1], since the first warp has access to the full tensor,
/// whereas / the other warps have access to replicated elements.
// SmallVector<unsigned>
...
#endif // TRITON_DIALECT_TRITON_INTEL_GPU_IR_DIALECT_H_
