...
class XPUBackend(BaseBackend):
    ...
    def make_llir(src, metadata, complier_options):
        ...
        # TritonGPU -> LLVM-IR
        ret = _triton.translate_triton_gpu_to_spirv(src, asdict(complier_options))
    ...
    def make_spvbin(src, metadata, opt):
        ...
        # return compile_ptx_to_cubin(src, ptxas, opt.enable_fp_fusion)
    ...
    def add_stages(self, stages, options):
        ...
        # stages["llir"] = lambda src, metadata: self.make_llir(src, metadata, compiler_options, linker_options)
        # stages["spirv"] = lambda src, metadata: self.make_spirv(src, metadata, compiler_options)
        # stages["spvbin"] = lambda src, metadata: self.make_spvbin(src, metadata, compiler_options)
