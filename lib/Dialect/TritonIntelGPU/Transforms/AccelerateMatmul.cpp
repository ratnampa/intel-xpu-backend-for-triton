...
SmallVector<unsigned, 2> getWarpsPerTile(tt::DotOp dotOp,
  ...
  uint32_t rowColRatio =
      mlir::ceil<uint32_t>(dpasCap.repeatCount, dpasCap.executionSize);
  uint32_t colRowRatio =
      mlir::ceil<uint32_t>(dpasCap.executionSize, dpasCap.repeatCount);
  do {
...
public:
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    unsigned opsPerChan = dpasCap.opsChanBitWidths / mmaElemBitWidths;
