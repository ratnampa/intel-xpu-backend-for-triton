...
struct LoadOpConversion
  ...
  matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
    ...
    if (isTensorPointerType(op.getPtr().getType())) {
      if (rewriteTensorPointerLoad(op, adaptor, rewriter).failed()) {
        // TODO: (johnlu) set the vector size > 1; Need to prove the memory is
        // contiguous on the fast change dim fallback to gather load.
        Type resultType = op.getType();
        auto tensorType = cast<RankedTensorType>(resultType);
...
struct StoreOpConversion
  ...
  matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
    ...
    if (isTensorPointerType(ptr.getType())) {
      if (rewriteTensorPointerStore(op, adaptor, rewriter).failed()) {
    ...
    assert(!maskElems.size() || valueElems.size() == maskElems.size());
